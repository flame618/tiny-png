 ...
 */
export async function startTiny(filePath: any, isRecursive = false) {
  if (!filePath) {
  ...
  if (lstat.isDirectory()) {
    // 处理文件夹
    const files: any = await getAllImageFiles(filePath, isRecursive);
    const promises: any[] = [];
 ...
 */
async function tinypng(imagePath: any) {
 ...
 * @param {string} imagePath 指定图片路径
 */
function uploadImage(imagePath: any) {
  return new Promise((resolve, reject) => {
    ...
    }, (res: { statusCode: number; statusMessage: string | undefined; setEncoding: (arg0: string) => void; on: (arg0: string, arg1: (buffer: any) => void) => void; }) => {
 ...
 */
function downloadImage(url: any, imagePath: any) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      headers: {
        "X-Forwarded-For": getRandomIP()
      }
    }, (res: { pipe: (arg0: any) => { (): any; new(): any; on: { (arg0: string, arg1: () => void): void; new(): any; }; }; }) => {
